...
#ifdef _WIN32
std::string wstring_to_utf8 (const std::wstring& ostr)
{
    // std::wstring_convert<std::codecvt_utf8<wchar_t>> myconv;
    // return myconv.to_bytes(str);
	
	std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> convert;
	std::string str = convert.to_bytes( ostr );
	return str;
}
int wmain( int argc, wchar_t* argv[] )
{
	...
	int ret = 1;
	std::vector<std::string> args_ws;
	for (int ai=0; ai<argc;ai++)
	{
		std::wstring ws = std::wstring ( argv[ai] );
		
		// std::string sstr( ws.begin(), ws.end() );
		// args_ws.push_back(sstr);		
		
		args_ws.push_back(wstring_to_utf8(ws));
